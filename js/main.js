...
const ahorcadoContainer = mainGame.querySelector(
...
class AhorcadoGame {
    ...
    localStorageCheck = () => {
        try {
            ...
            if (
                actualLocalStorage.length < len ||
                !localStorage.ahorcadoWords
                    .slice(0, -1)
                    .includes(JSON.stringify(starterWords).slice(0, -1))
            ) {
    ...
    tryLetter = (char) => {
        ...
        if (this.tries.has(key)) {
            document
                ...
                .forEach((letter) => this.ShakeAnimation(letter));
            return false;
        }
        ...
        if (index < 0) {
            ...
            this.failsNumber++;
            let part = personGame.querySelector(
                `.person__part-${this.failsNumber}`
            );
        ...
        while (index >= 0) {
            ...
            let targetChar = triesContainer.querySelector(
                `.char--${index + 1}`
            );
    ...
    newPersonPartAnimation = (part) => {
        ...
        let interval = setInterval(() => {
            ...
            if (
                ...
                part.y.baseVal.value - 5 <= initialY
            ) {
